...
let A = new Array(N);
for (let i = 0; i < N; i++) {
  A[i] = input[1 + i].split(" ").map((el) => parseInt(el));
}

//  나무 Board 세팅 (각 요소 : [나이, 나이, ...])
      ...
      if (trees[j][k].length === 0) continue;
        ...
        if (trees[j][k][0] > 0) {
          if (basics[j][k] >= trees[j][k][0]) {
            ...
            //나무가 양분을 먹지 못하고 죽음
            trees[j][k][0] = -trees[j][k][0];
          }
        }
      } else {
        //나무가 한 그루 이상일 때
        trees[j][k].forEach((tree, index) => {
          if (tree > 0) {
            if (basics[j][k] >= tree) {
              //나무가 양분을 먹음
              basics[j][k] -= tree;
      ...
      if (trees[j][k].length === 0) continue;
        ...
        trees[j][k].forEach((tree) => {
          ...
          if (tree < 0) {
            ...
            trees[j][k] = trees[j][k].filter((item) => item !== tree);
          }
        });
      }
    }
